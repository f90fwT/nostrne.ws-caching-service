...
export async function calculate() {
    ...
    for (let i = 0; i < notes.length; i++) {
        ...
        let algoUpvoteScore = calculateRanking(upvoteCount, Date.now() - Number(note.CreatedAt), 0.6).toFixed(32);
        let algoZapsatsScore = calculateRanking(zapSats, Date.now() - Number(note.CreatedAt), 0.6).toFixed(32);
